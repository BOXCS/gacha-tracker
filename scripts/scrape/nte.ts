...
const API_URL = 'https://neverness-to-everness.fandom.com/api.php?action=query&prop=pageimages&pithumbsize=200&generator=categorymembers&gcmtitle=Category:Characters&gcmlimit=500&format=json';
const OUTPUT_PATH = path.resolve(__dirname, '../../data/characters/nte.json');

async function scrapeNTE() {
  ...
  try {
    ...
    Object.values(pages).forEach((page: any) => {
      ...
      characters.push({
        id: `nte_${name.toLowerCase().replace(/[^a-z0-9]/g, '_')}`,
        name,
        game: 'nte',
        element: 'Unknown',
        role: 'Unknown',
    ...
    console.log(`Berhasil mengekstrak ${characters.length} karakter NTE.`);
